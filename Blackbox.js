...
Blackbox.prototype.navigatePlaylist = function(playlist, keepPosition)
{
    ...
    if (playlist)
        for (i = 0; i < playlist.length; ++i) {
            ...
            entryText = entryIndex+': '+entryPath;
            menuOptions.push(entryIsPlaying ? {
                prefix: entryIndex+': ',
                menuText: entryPath,
                // Matches the "active" green/glyph used by the mpv-scripts
                // Lua scroll-list menus (auto4k.lua etc.), not AssFormat's
                // washed-out Ass.green.
                style: function(output) { return Ass.color('00FF00', output); },
                glyph: '▶'
            } : entryText);
