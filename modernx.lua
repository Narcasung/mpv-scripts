...
function tick()
    ...
    if (state.idle) then
        ...
        -- skip the idle logo/text while a scroll-list.lua-based menu
        -- (track-list.lua/recent.lua/auto4k.lua) is on screen, since it's
        -- drawn on a separate overlay that would otherwise sit on top of it
        local list_open = mp.get_property_native('user-data/mpv-scripts/list-open', false)

        local ass = assdraw.ass_new()
        -- mpv logo
        if user_opts.idlescreen and not list_open then
        ...
        -- Santa hat
        if is_december and user_opts.idlescreen and not user_opts.greenandgrumpy and not list_open then
