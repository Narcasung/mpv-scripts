...
var SelectionMenu = function(settings)
{
    ...
    // closes this menu when another mpv script's list opens (see the matching
    // broadcast in _showMenu() below); coordinates with the scroll-list.lua
    // menu engine used by track-list.lua/recent.lua/auto4k.lua so only one
    // on-screen menu is ever open at a time
    var self = this;
    mp.register_script_message('mpv-scripts-close-other-lists', function(sender) {
        if (sender !== mp.get_script_name() && self.isMenuActive())
            self.hideMenu();
    });
};
...
SelectionMenu.prototype._showMenu = function()
{
    ...
    if (!this.isMenuActive()) {
        ...
        // paired with the listener registered in the constructor above
        mp.commandv('script-message', 'mpv-scripts-close-other-lists', mp.get_script_name());
